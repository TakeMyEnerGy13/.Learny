...
module.exports = {
  ...
  theme: {
    fontFamily: {
      ...
      display: ['Geist', '-apple-system', 'BlinkMacSystemFont', 'Helvetica Neue', 'sans-serif'],
      // Learny's decorative script (loaded in static/index.html). For single
      // accented words inside otherwise plain type — never for running copy or
      // anything a visitor has to read fast. Use the `.learny-script` helper in
      // agent/theme.css rather than this utility alone: a script face needs its
      // own size and tracking compensation next to Geist.
